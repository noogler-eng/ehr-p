 ...
 */
router.post("/records/submit-visit", upload.array('images', 5), async (req, res) => {
    ...
    try {
        ...
        const enhancedPrescriptions = parsedPrescriptions.map((p) => ({
            ...p,
        ...
        if (isNewRecord) {
            ...
            record = await EHR.create({
                ...
                prediction: {
                    ...
                    predicted_conditions: (xgboostPrediction.predicted_conditions || []),
            ...
            txHash = await createBlockchainTransaction(doctorAddress, 'CREATE_RECORD', record._id, { patientAddress, riskLevel: finalRiskLevel, action: 'INITIAL_CREATE' });
            ...
            existingRecord.prescriptions = mergedPrescriptions;
            existingRecord.images = mergedImages;
            if (doctorRecommendations) {
                existingRecord.doctorRecommendations = doctorRecommendations;
            }
            // Add to update history
            txHash = await createBlockchainTransaction(doctorAddress, 'UPDATE_RECORD', existingRecord._id, { action: 'DOCTOR_UPDATE', newSymptoms: parsedSymptoms.length, newPrescriptions: enhancedPrescriptions.length });
        ...
        if (enhancedPrescriptions.length > 0) {
            const newMeds = enhancedPrescriptions.map((p) => p.medicineName).filter((m) => m);
            await User.findOneAndUpdate({ walletAddress: patientAddress }, {
        ...
        res.status(500).json({ error: "Clinical processing failed", details: err.message });
    }
});
/**
 ...
 */
router.get("/records/history/:address", async (req, res) => {
    try {
 ...
 */
router.put("/records/update/:recordId", upload.array('images', 5), async (req, res) => {
    try {
        ...
        const enhancedPrescriptions = parsedPrescriptions ? parsedPrescriptions.map((p) => ({
            ...p,
            prescribedBy: doctorAddress,
            prescribedDate: new Date()
        })) : null;
        ...
        if (enhancedPrescriptions) {
            ...
            record.prescriptions = [...existingPrescriptions, ...enhancedPrescriptions];
        }
        ...
        const updatedFields = [];
        if (doctorRecommendations)
            updatedFields.push('doctorRecommendations');
        if (enhancedPrescriptions)
            updatedFields.push('prescriptions');
        if (imageCount > 0)
            updatedFields.push('images');
        const txHash = await createBlockchainTransaction(doctorAddress, 'UPDATE_RECORD', record._id, {
            updatedFields,
            timestamp: new Date(),
 ...
 */
router.get("/doctor/my-patients/:doctorAddress", async (req, res) => {
    try {
 ...
 */
router.get("/patient/my-doctors/:patientAddress", async (req, res) => {
    try {
 ...
 */
router.get("/pharmacist/registry", async (_req, res) => {
    try {
        const patientAddrs = await EHR.distinct("patientAddress");
 ...
 *         description: Success
 */
router.get("/pharmacy/prescriptions/:patientAddress", async (req, res) => {
    try {
        const patientAddress = req.params.patientAddress;
        if (!patientAddress) {
 ...
 */
router.get("/blockchain/wallet/:address", async (req, res) => {
    try {
        const address = req.params.address;
 ...
 * @returns {object} Updated record
 */
router.put("/records/patient-update/:recordId", upload.array('images', 5), async (req, res) => {
    try {
        const { recordId } = req.params;
        // Handle case where req.body might be undefined or empty
        if (!req.body || Object.keys(req.body).length === 0) {
        ...
        const updatedFields = [];
        if (symptoms)
            updatedFields.push('symptoms');
        if (patientNotes)
            updatedFields.push('patientNotes');
        if (imageCount > 0)
            updatedFields.push('images');
        const txHash = await createBlockchainTransaction(patientAddress, 'UPDATE_RECORD', record._id, {
            updatedFields,
            updatedBy: 'patient',
            timestamp: new Date()
        });
 ...
 */
router.post("/records/reanalyze/:recordId", async (req, res) => {
    try {
        const { recordId } = req.params;
        ...
        const xgboostPrediction = await predictDisease({
            vitals: {
                ...
                ecgResult: record.clinicalData.vitals.ecgResult || 'Normal'
            },
            clinicalMarkers: record.clinicalData.clinicalMarkers || {},
            age: patientAge
        });
        ...
        const prompt = `
            ...
            - Clinical: Chest Pain Type (N/A), ECG Results (${record.clinicalData.vitals.ecgResult || 'Normal'}).
 ...
 */
router.get("/blockchain/explorer", async (req, res) => {
    try {
 ...
 */
router.get("/blockchain/search/:query", async (req, res) => {
    try {
...
router.post("/access/grant", async (req, res) => {
    try {
        ...
        const txHash = await createBlockchainTransaction(patientAddress, 'GRANT_ACCESS', record?._id, { grantedTo: doctorAddress, doctorName: doctor.name, timestamp: new Date() });
        res.json({
            success: true,
            txHash,
...
router.post("/access/revoke", async (req, res) => {
    try {
        ...
        const record = await EHR.findOne({ patientAddress });
        const txHash = await createBlockchainTransaction(patientAddress, 'REVOKE_ACCESS', record?._id, { revokedFrom: doctorAddress, timestamp: new Date() });
        res.json({
            success: true,
            txHash,
 ...
 */
router.get("/access/history/:address", async (req, res) => {
    try {
        const address = req.params.address;
 ...
 */
router.get("/chat/list/:address", async (req, res) => {
    try {
        const address = req.params.address;
        const user = await User.findOne({ walletAddress: address });
        let chats;
        if (user?.role === 'Doctor') {
            chats = await Chat.find({ 'participants.doctorAddress': address }).sort({ lastMessage: -1 });
        }
        else {
            chats = await Chat.find({ 'participants.patientAddress': address }).sort({ lastMessage: -1 });
        }
        const enriched = await Promise.all(chats.map(async (chat) => {
            const otherAddress = user?.role === 'Doctor'
                ? chat.participants?.patientAddress
                : chat.participants?.doctorAddress;
            const otherUser = await User.findOne({ walletAddress: otherAddress }).select('name role');
            const unreadCount = chat.messages.filter((m) => !m.read && m.sender !== address).length;
            return {
                _id: chat._id,
                otherAddress,
                ...
                lastText: chat.messages.length > 0 ? chat.messages[chat.messages.length - 1].text : '',
                unreadCount,
 ...
 */
router.put("/chat/read/:doctorAddress/:patientAddress/:readerAddress", async (req, res) => {
    try {
        const doctorAddress = req.params.doctorAddress;
        const patientAddress = req.params.patientAddress;
        const readerAddress = req.params.readerAddress;
 ...
 */
router.get("/chat/:doctorAddress/:patientAddress", async (req, res) => {
    try {
        const doctorAddress = req.params.doctorAddress;
        const patientAddress = req.params.patientAddress;
        let chat = await Chat.findOne({
        ...
        const [doctor, patient] = await Promise.all([
            User.findOne({ walletAddress: doctorAddress }).select('name'),
            User.findOne({ walletAddress: patientAddress }).select('name')
        ]);
        res.json({
            success: true,
            chat,
 ...
 */
router.get("/followup/list/:address", async (req, res) => {
    try {
        const address = req.params.address;
        const user = await User.findOne({ walletAddress: address });
...
router.put("/followup/update/:id", async (req, res) => {
    try {
        ...
        const followUp = await FollowUp.findByIdAndUpdate(req.params.id, { status }, { new: true });
 ...
 */
router.post("/medication/log", async (req, res) => {
    try {
        ...
        const log = await MedicationLog.findOneAndUpdate({ patientAddress, medicineName, date }, { patientAddress, medicineName, dosage, date, status, time, notes, loggedAt: new Date() }, { upsert: true, new: true });
 ...
 */
router.get("/vitals/history/:patientAddress", async (req, res) => {
    try {
        const patientAddress = req.params.patientAddress;
        const history = await VitalsHistory.find({ patientAddress })
            .sort({ date: 1 })
            .limit(50);
 ...
 */
router.post("/vitals/seed-demo/:patientAddress", async (req, res) => {
    try {
        const patientAddress = req.params.patientAddress;
        // Check if demo data already exists
        const existing = await VitalsHistory.countDocuments({ patientAddress });
        if (existing >= 8) {
...
//# sourceMappingURL=ehrRoutes.js.map